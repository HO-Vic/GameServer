...
void AcceptEvent::Start(OverlappedEx* overlappedEx) {
  ...
  bool isSuccess = AcceptEx(m_listenSocket, m_clientSocket, &m_connInfo, 0, sizeof(m_connInfo.localInfo), sizeof(m_connInfo.remoteInfo), nullptr, reinterpret_cast<LPOVERLAPPED>(overlappedEx));
}
