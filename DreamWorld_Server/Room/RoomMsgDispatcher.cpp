...
void RoomMsgDispatcher::OnKeyDown(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnKeyUp(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  const DreamWorld::CLIENT_PACKET::MovePacket* recvPacket = reinterpret_cast<const DreamWorld::CLIENT_PACKET::MovePacket*>(message);
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
...
void RoomMsgDispatcher::OnStopMove(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnRotate(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnExecuteSkill_Q(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnExecuteSkill_Q_Float3(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnExecuteSkill_E(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnExecuteSkill_E_Float3(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnInputQ(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnInputE(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnMouseInput(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnExecutePowerAttack(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnExecuteCommonAttack(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
...
void RoomMsgDispatcher::OnStageChangeBoss(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnForceGameEnd(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnGameEndOk(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
  auto userSession = std::static_pointer_cast<DreamWorld::Session>(sessionPtr);
  auto possessObject = userSession->GetPossessCharacter();
  auto roomRef = userSession->GetRoom();
  if (nullptr == possessObject || nullptr == roomRef) {
...
void RoomMsgDispatcher::OnTimeSyncReq(sh::IO_Engine::TCP_SessionBasePtr sessionPtr, BYTE* message) {
