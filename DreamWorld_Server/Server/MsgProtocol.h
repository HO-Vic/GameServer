...
struct PacketHeader
  ...
  unsigned char type;
  PacketHeader(const uint16_t size, unsigned char type)
...
struct ReConnectPacket : public PacketHeader {
  ...
  ReConnectPacket(const std::wstring& name, const char& type = static_cast<char>(TYPE::RECONN))
      : PacketHeader(type, sizeof(ReConnectPacket)) {
    StringCchCopyW(nickName, NAME_SIZE, name.c_str());
  }
};

struct MovePacket : public PacketHeader {  // 시간과 진행 방향, 이동 입력
  DIRECTION direction;
...
struct LoginPacket : public PacketHeader {
  char id[NAME_SIZE];
  char pw[NAME_SIZE];
  LoginPacket(const char& type = static_cast<char>(TYPE::LOGIN))
...
struct MouseInputPacket : public PacketHeader {
  ...
  MouseInputPacket(const char& type = static_cast<char>(TYPE::MOUSE_INPUT))
      : PacketHeader(type, sizeof(MouseInputPacket)) {
  }
...
struct CommonSkillPacket : public PacketHeader {
  CommonSkillPacket(const char& type, const short& size = sizeof(CommonSkillPacket))
      : PacketHeader(type, size) {
  }
};

struct FloatDataSkillPacket : public CommonSkillPacket {
  FloatDataSkillPacket(const char& type, const DirectX::XMFLOAT3& floatData)
      : CommonSkillPacket(type, sizeof(FloatDataSkillPacket)), floatData(floatData) {
  }
  DirectX::XMFLOAT3 floatData;
};
...
struct GameState_BOSS : public GameState_Base {
  BossState bossState;
  GameState_BOSS()
      : GameState_Base(static_cast<char>(SERVER_PACKET::TYPE::GAME_STATE_BOSS), sizeof(GameState_BOSS)) {
  }
};
...
struct GameState_STAGE : public GameState_Base {
  ...
  GameState_STAGE()
      : GameState_Base(static_cast<char>(SERVER_PACKET::TYPE::GAME_STATE_STAGE), sizeof(GameState_STAGE)) {
  }
};
...
struct BossStageInitPacket : public PacketHeader {
  DirectX::XMFLOAT3 bossPosition;  // 보스 시작할때 위치
  DirectX::XMFLOAT3 bossLookVector;
  float bossHp;
  PlayerState userState[4];  // 유저 데이터
  BossStageInitPacket()
      : PacketHeader(static_cast<char>(TYPE::START_STAGE_BOSS), sizeof(BossStageInitPacket)) {
  }
};

struct BossMoveDestnationPacket : public PacketHeader {
  DirectX::XMFLOAT3 destination;
  BossMoveDestnationPacket(const DirectX::XMFLOAT3& destination)
      : PacketHeader(static_cast<char>(TYPE::BOSS_MOVE_DESTINATION), sizeof(BossMoveDestnationPacket)), destination(destination) {
  }
};

struct BossStopPacket : public PacketHeader {
  BossStopPacket()
      : PacketHeader(static_cast<char>(TYPE::BOSS_STOP), sizeof(BossStopPacket)) {
  }
};

struct BossOnSameNodePacket : public PacketHeader {
  DirectX::XMFLOAT3 destination;
  BossOnSameNodePacket(const DirectX::XMFLOAT3& destination)
      : PacketHeader(static_cast<char>(TYPE::BOSS_ON_SAME_NODE), sizeof(BossOnSameNodePacket)), destination(destination) {
  }
};
