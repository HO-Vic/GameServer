...
void GameObject::SetLook(const XMFLOAT3& lookVector) {
  ...
  m_worldMatrix._11 = newRightVector.x;
  m_worldMatrix._12 = newRightVector.y;
  m_worldMatrix._13 = newRightVector.z;
  m_worldMatrix._21 = upVector.x;
  m_worldMatrix._22 = upVector.y;
  m_worldMatrix._23 = upVector.z;
  m_worldMatrix._31 = newLookVector.x;
  m_worldMatrix._32 = newLookVector.y;
  m_worldMatrix._33 = newLookVector.z;
}

LiveObject::LiveObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, std::shared_ptr<RoomBase>& roomRef)
...
std::optional<std::pair<bool, XMFLOAT3>> LiveObject::CollideLiveObject(const XMFLOAT3& nextPosition, const XMFLOAT3& moveVector, const float& elapsedTime, const bool& isSlidingPosition) {
  ...
  std::vector<std::shared_ptr<GameObject>>& gameObjects = roomRef->GetLiveObjects();
  for (auto& gameObject : gameObjects) {
    auto liveObject = std::static_pointer_cast<LiveObject>(gameObject);
    // 자기 자신이라면 넘어감
    if (liveObject == shared_from_this()) {
      continue;
    }
    if (!liveObject->IsAlive()) {
      continue;
    }
    float distance = liveObject->GetDistance(nextPosition);
    if (liveObject->GetCollision().Radius + m_collisionSphere.Radius < distance) {
      continue;
    }
  ...
  if (slidingDotLookResult < 0) {
    slidingVector = Vector3::ScalarProduct(slidingVector, -1.0f, false);  // 0보다 작다는건, 반대 방향을 의미
  }
...
MoveObject::MoveObject(const float& moveSpeed, const float& boundingSize, std::shared_ptr<RoomBase>& roomRef)
    : GameObject(boundingSize, roomRef), m_moveSpeed(moveSpeed), m_lastUpdateTime(std::chrono::high_resolution_clock::now()) {
}
