...
class DirectionSkill : public DirectionSkillBase {
 public:
  DirectionSkill(std::shared_ptr<WarriorObject> warriorRef, const XMFLOAT3& direction)
      : DirectionSkillBase(direction), warriorRef(warriorRef) {
  }
...
class LongSwordSkill : public DirectionSkill {
 public:
  LongSwordSkill(std::shared_ptr<WarriorObject> warriorRef, const XMFLOAT3& direction)
      : DirectionSkill(warriorRef, direction) {
  }
...
{
 public:
  CommonAttack(std::shared_ptr<WarriorObject> warriorRef, const XMFLOAT3& direction, const int& power)
      : DirectionSkill(warriorRef, direction), power(power) {
  }
  virtual void Execute() override;
...
class WarriorObject : public MeleeCharacterObject {
 public:
  WarriorObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, std::shared_ptr<RoomBase>& roomRef);
