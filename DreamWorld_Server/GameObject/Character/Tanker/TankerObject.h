...
class TankerRef {
 public:
  TankerRef(std::shared_ptr<TankerObject> tankerRef)
      : tankerRef(tankerRef) {
  }
...
class DirectionSkill : public DirectionSkillBase, public TankerRef {
 public:
  DirectionSkill(std::shared_ptr<TankerObject> tankerRef, const XMFLOAT3& direction)
      : TankerRef(tankerRef), DirectionSkillBase(direction) {
  }
};

class ThunderHammerSkill : public DirectionSkill {
 public:
  ThunderHammerSkill(std::shared_ptr<TankerObject> tankerRef, const XMFLOAT3& direction)
      : DirectionSkill(tankerRef, direction) {
  }
...
{
 public:
  CommonAttack(std::shared_ptr<TankerObject> tankerRef, const XMFLOAT3& direction)
      : DirectionSkill(tankerRef, direction) {
  }
  virtual void Execute() override;
};
...
class TankerObject : public MeleeCharacterObject {
 public:
  TankerObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, std::shared_ptr<RoomBase>& roomRef);
