...
class ArcherRef {
 public:
  ArcherRef(std::shared_ptr<ArcherObject> archerRef)
      : archerRef(archerRef) {
  }
...
{
 public:
  CommonAttack(std::shared_ptr<ArcherObject> archerRef, const XMFLOAT3& direction, const int& power)
      : DirectionSkill(archerRef, direction), power(power) {
  }
  virtual void Execute() override;
...
class ArcherObject : public RangedCharacterObject {
 public:
  ArcherObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, std::shared_ptr<RoomBase>& roomRef);
