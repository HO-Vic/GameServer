...
CharacterObject::CharacterObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, const float& attackDamage, std::shared_ptr<RoomBase>& roomRef, const ROLE& role)
    : LiveObject(maxHp, moveSpeed, boundingSize, roomRef), m_currentDirection(static_cast<char>(DIRECTION::IDLE)), m_leftMouseInput(false), m_rightMouseInput(false), m_Shield(0.0f), m_activeShield(false), m_commonAttackDamage(attackDamage), m_role(role) {
...
void CharacterObject::RecvDirection(const DIRECTION& dir, const bool& apply) {
  if (apply) {
    m_applyDIrection |= static_cast<char>(dir);
  } else {
    m_applyDIrection ^= static_cast<char>(dir);
  }
...
std::optional<const XMFLOAT3> MeleeCharacterObject::GetMoveVector() const {
  ...
  switch (moveDirection) {
    case FRONT_BIT:
      rotateAngle = 0.0f;
      break;
    case FRONT_BIT | LEFT_BIT:
      rotateAngle = 45.0f;
      break;
    case LEFT_BIT:
      rotateAngle = 90.0f;
      break;
    case BACK_BIT | LEFT_BIT:
      rotateAngle = 135.0f;
      break;
    case BACK_BIT:
      rotateAngle = 180.0f;
      break;
    case BACK_BIT | RIGHT_BIT:
      rotateAngle = 225.0f;
      break;
    case RIGHT_BIT:
      rotateAngle = 270.0f;
      break;
    case FRONT_BIT | RIGHT_BIT:
      rotateAngle = 315.0f;
      break;
    default:
      return std::nullopt;
...
RangedCharacterObject::RangedCharacterObject(const float& maxHp, const float& moveSpeed, const float& boundingSize, const float& attackDamage, std::shared_ptr<RoomBase>& roomRef, const ROLE& role)
    : CharacterObject(maxHp, moveSpeed, boundingSize, attackDamage, roomRef, role) {
}
